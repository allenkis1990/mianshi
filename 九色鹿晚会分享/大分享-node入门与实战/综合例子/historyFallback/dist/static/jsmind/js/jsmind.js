...
(function($w) {
    ...
    jm.prototype = {
        ...
        dblclick_handle: function(e) {
            ...
            if (this.get_editable()) {
                var element = e.target || event.srcElement;
                var nodeid = this.view.get_binded_nodeid(element);
                //根节点不让编辑（为了防止随意更改学科名字）
                if (!!nodeid && !this.get_node(nodeid).isroot) {
        ...
        addbrother: function(selected_node, nodeid, topic) {
            if (!!selected_node && !selected_node.isroot) {
                var node = this.insert_node_after(selected_node, nodeid, topic);
                if (!!node) {
                    this.select_node(nodeid);
                    this.begin_edit(nodeid);
                }
    ...
    jm.view_provider.prototype = {
        ...
        select_node: function(node) {
            ...
            if (!!node) {
                //不是根节点并且不是新添加的字节的
                if (node.id != 0 && !isNaN(node.id)) {
                    Bus.$emit('choosePoint', node)
                } else {
                    Bus.$emit('choosePoint', '')
                }
    ...
    jm.shortcut_provider.prototype = {
        init: function() {
            ...
            this.handles['printscreen'] = this.handle_printscreen;
            this.handles['zoomIn'] = this.handle_zoomIn;
            this.handles['zoomOut'] = this.handle_zoomOut;
        ...
        handle_zoomIn: function(_jm) {
            _jm.view.zoomIn()
        },
        handle_zoomOut: function(_jm) {
            _jm.view.zoomOut()
        },
        handle_addchild: function(_jm, e) {
            ...
            if (!!selected_node) {
                ...
                var node = _jm.add_node(selected_node, nodeid, '知识点');
                if (!!node) {
        ...
        handle_printscreen: function(_jm, e) {
        ...
        handle_addbrother: function(_jm, e) {
            ...
            if (!!selected_node && !selected_node.isroot) {
                var nodeid = jm.util.uuid.newid();
                var node = _jm.insert_node_after(selected_node, nodeid, '知识点');
                if (!!node) {
                    _jm.select_node(nodeid);
                    _jm.begin_edit(nodeid);
                }
